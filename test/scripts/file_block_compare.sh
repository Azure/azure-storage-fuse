...
echo "| File Size (MB) | Block Cache | File Cache | NFS |" > cpu_usage_write.txt
echo "| -- | -- | -- | -- |" >> cpu_usage_write.txt
...
do
    ...
    do
        ...
        sed -i "${sed_line}s/$/ ${captured_output} |/" cpu_usage_write.txt
        (( sed_line++ ))
        sleep 2
    done
...
echo "| Thread | Block Size (MB) | File Size (MB) | Block Cache | File Cache | AML | NFS |" > cpu_usage_read.txt
echo "| -- | -- | -- | -- | -- | -- | -- |" >> cpu_usage_read.txt
# Generate the test case data
while IFS=, read -r thread block file; do
    echo "| ${thread} | ${block} | ${file} |" >> $outputPath
    echo "| ${thread} | ${block} | ${file} |" >> cpu_usage_read.txt
...
do
    ...
    while IFS=, read -r thread block file; do
        ...
        sed -i "${sed_line}s/$/ ${captured_output} |/" cpu_usage_read.txt
        (( sed_line++ ))
    done < <(tail -n +3 ./test/scripts/fio_tests.csv)
