...
- In addition to the FUSE module options; blobfuse offers following options:
    ...
    * [OPTIONAL] **--max-concurrency=20** : option to override default number of concurrent storage connections, default=12, please note that blobfuse caps this at 40. So if the user specifies a value between 0 to 40, the users value will override the default. However, if the user specifies a value > 40 blobfuse will still only allow 40 concurrent connections. Blobfuse does not recommend multi-instance write operations. It does allow concurrent write operations to different files in the same instance.
