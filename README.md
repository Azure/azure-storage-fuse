...
Blobfuse is stable, and is supported by Azure Storage given that it is used within its limits documented here. Blobfuse supports both reads and writes however, it does guarantee continuous sync on data written to storage using other APIs oor other mounts of blobfuse. For data integrity it is recommended that multiple sources do not modify the same blob. Please submit an issue [here](https://github.com/azure/azure-storage-fuse/issues) for any issues/requests/questions.
...
- In addition to the FUSE module options; blobfuse offers following options:
    ...
    * [OPTIONAL] **--cache-size-mb=1000** : option to setup the cache-size in MB. Default will be 80% of the available memory, eviction will happen beyond that. Use this option to lower the cache size or increase it. This option is only available after version 1.3.1.
     ...
     * [OPTIONAL] **--max_retry_delay=60** : When a retry is attempted, maximum number of seconds allowed between two retries.
     * [OPTIONAL] **--retry_delay_factor=1.2** : When exponential retry is attempted, factor in seconds by which retry interval grows exponentiallly.
