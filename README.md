...
- In addition to the FUSE module options; blobfuse offers following options:
    ...
    * [OPTIONAL] **--background-download=false** : Instead of downloading the file in 'open' system call, download it in background post 'open'. Follow up 'read'/'write' calls will wait for download to complete. This option is only available from version 1.4.0
    * [OPTIONAL] **--evict-on-sync=false** : When file cache timeout is set to a higher value and user wants to force evict a file from cache use the sync command to do the same. If set to false, sync command on file is ignored. This option is only available from version 1.4.0
