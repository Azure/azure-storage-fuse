...
- Mount with workflow optimization
    * blobfuse2 mount \<mount path\> --config-file=\<config file\> --workflow=training
    * blobfuse2 mount \<mount path\> --config-file=\<config file\> --workflow=serving
    * blobfuse2 mount \<mount path\> --config-file=\<config file\> --workflow=checkpointing
...
- General options
    ...
    * `--workflow=<training|serving|checkpointing>`: Auto-configure cache pipeline and settings based on workflow type. This option automatically configures the component pipeline and optimal cache settings for common use cases:
        - **training**: Sets pipeline to use file_cache for better read performance with training datasets. Configures large cache size (8GB) and long cache timeout (7200 seconds/2 hours) for file_cache and attr_cache.
        - **serving**: Sets pipeline to use xload (preload) for model serving workloads. Downloads all files on mount, enforces read-only mode, and configures moderate attr_cache timeout (3600 seconds/1 hour) and libfuse settings (300 seconds).
        - **checkpointing**: Sets pipeline to use block_cache optimized for writing large checkpoint files. Configures larger blocks (64MB), high parallelism (128), disables kernel cache for immediate writes, and sets shorter libfuse timeouts (60 seconds).
        - Note: Workflow only sets the component pipeline if no explicit pipeline or cache flags are provided. Individual configuration values can be overridden with explicit flags.
