...
func releaseAllBuffersForFile(file *File) {
	...
	for _, blk := range file.blockList.list {
		...
		if ok1, ok2 := btm.removeBufferDescriptor(bufDesc, true /*strict*/); !ok1 || !ok2 {
			...
			panic(fmt.Sprintf("releaseAllBuffersForFile: Failed to remove bufferIdx: %d, refCnt: %d, for blockIdx: %d of file %s from buffer table manager, isRemovedFromBufMgr: %v, isReleasedToFreeList: %v",
				bufDesc.bufIdx, bufDesc.refCnt.Load(), blk.idx, file.Name, ok1, ok2))
		}
