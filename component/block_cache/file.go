...
func (f *File) write(options *internal.WriteFileOptions) error {
	...
	retry:
		...
		if blockIdx < blockListLen {
			...
			// 	lastBlockIdx := getBlockIndex(fileSize - 1)
			// 	if lastBlockIdx < blockListLen {
			// 		lastBlock := f.blockList.list[lastBlockIdx]
			// 		f.mu.Unlock()

			// 		bufDesc, status, err := GetOrCreateBufferDescriptor(lastBlock, true /*doesRead*/, true /*sync*/)
			// 		if err != nil {
			// 			f.err.Store(err.Error())
			// 			// Decrement the write wait group on error
			// 			f.pendingWriters.Done()
			// 			log.Err("File::write: [SPARSE] Failed to get buffer descriptor for last blockIdx: %d during write at file: %s, offset: %d, [%v]",
			// 				lastBlock.idx, f.Name, options.Offset, err)
			// 			return err
			// 		}

			// 		if bufDesc == nil {
			// 			panic(fmt.Sprintf("File::write: [SPARSE] No buffer descriptor found for last blockIdx: %d during write at file: %s, offset: %d",
			// 				lastBlock.idx, f.Name, options.Offset))
			// 		}

			// 		lastBlock.state = localBlock
			// 		bufDesc.dirty.Store(true)

			// 		log.Debug("File::write: [SPARSE] Got buffer descriptor for last blockIdx: %d during write at file: %s, offset: %d, status: %v",
			// 			lastBlock.idx, f.Name, options.Offset, status)
			// 		// Release the buffer descriptor, this should not get released back to free list as we are not removing it from buffer table manager.
			// 		if ok := bufDesc.release(); ok {
			// 			panic(fmt.Sprintf("File::write: [SPARSE] Released bufferIdx: %d for last blockIdx: %d back to free list after write at file: %s, offset: %d",
			// 				bufDesc.bufIdx, lastBlock.idx, f.Name, options.Offset))
			// 		}
		...
		for i := noOfBlocks; i < len(f.blockList.list); i++ {
			...
			if bufDesc != nil {
				...
				if ok1, ok2 := btm.removeBufferDescriptor(bufDesc, false /*strict*/); !ok1 {
					panic(fmt.Sprintf("File::truncate: Removed bufferIdx: %d for blockIdx: %d from buffer table manager during truncate at file: %s, isRemovedFromBufMgr: %v, isReleasedToFreeList: %v, refCnt: %d",
						bufDesc.bufIdx, blk.idx, f.Name, ok1, ok2, bufDesc.refCnt.Load()))
				}
			}
		}
