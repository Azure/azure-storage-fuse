...
func (f *File) flush(takeFileLock bool) error {
	...
	uploadZeroBlock := func(blk *block, isLastBlock bool) error {
		...
		log.Debug("File::flush: Uploading zero block for blockIdx: %d during flush at file: %s",
			blk.idx, f.Name)

		offsetInsideBlock := int64(bc.blockSize)

		if isLastBlock {
			offsetInsideBlock = convertOffsetIntoBlockOffset(f.size)
...
func (f *File) truncate(options *internal.TruncateFileOptions) error {
		...
		for i := noOfBlocks; i < len(f.blockList.list); i++ {
			...
			if bufDesc != nil {
				...
				if ok1, ok2 := btm.removeBufferDescriptor(bufDesc, false /*strict*/); !ok1 || !ok2 {
					panic(fmt.Sprintf("File::truncate: Removed bufferIdx: %d for blockIdx: %d from buffer table manager during truncate at file: %s",
						bufDesc.bufIdx, blk.idx, f.Name))
				}
			}
		}
	...
	if len(f.blockList.list) > 0 {
		...
		bufDesc.contentLock.Lock()
		offsetInsideBlock := convertOffsetIntoBlockOffset(f.size)
		copy(bufDesc.buf[offsetInsideBlock:], freeList.bufPool.zeroBuf[:])
		bufDesc.contentLock.Unlock()
