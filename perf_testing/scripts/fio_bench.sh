...
# test of parameters we are looking for ("bandwidth" or "latency")
param=$3

# Directory where output logs will be generated by fio
output="./${test_name}_${param}"
...
execute_test() {
  ...
  then
    ...
    jq -n 'inputs.jobs[] | if (."job options".rw == "read") 
      ...
      else {name: .jobname, value: (.write.lat_ns.mean / 1000000), unit: "milliseconds"} end' ${output}/${job_name}trial*.json | tee ${output}/${job_name}_summary.json
  fi
}
